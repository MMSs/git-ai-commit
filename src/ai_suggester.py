...
class AISuggester:
    ...
    def _build_prompt(self, diff: str, context: Dict[str, str], mode: str) -> str:
        ...
        multi_line_prompt = f"""Generate a multi-line git commit message for the previously mentioned staged changes.

The commit message should include:
1. A short summary ({self.config.get("suggestion.max_length")} characters or less)
...
"""
        single_line_prompt = f"""Generate a single-line git commit message for the previously mentioned staged changes.

The commit message should be on one line, concise, and ideally under {self.config.get("suggestion.max_length")} characters.
